...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "plutus", type: "shell", :path => "NixOS-Plutus-Scripts/plutus-cohort03-week09.bash", privileged: false
  ...
  config.vm.provision "shell", inline: <<-SHELL
    ...
    echo "The code for the pioneer program can be located in /opt," 
    echo "to access it from host, you can copy it to /vagrant"
    echo "Example commands to compile a project (from the vm shell):"
    echo "  cd /opt/plutus"
    echo "  nix-shell"
    echo "  cd /opt/plutus-pioneer-program/code/week01/"
